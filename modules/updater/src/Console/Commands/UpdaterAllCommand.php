...
class UpdaterAllCommand extends Command
{
    protected $signature = 'updater:all {--queue : Dispatch to queue instead of running immediately}';

    protected $description = 'Update all mapped modules via Composer';

    public function handle(UpdaterExecutionService $service): int
    {
        $execution = $service->createExecution(null, 'all');

        if ((bool) $this->option('queue')) {
            RunUpdaterExecutionJob::dispatch($execution->id);
            $this->info(sprintf('Queued updater execution #%d for all modules.', $execution->id));
