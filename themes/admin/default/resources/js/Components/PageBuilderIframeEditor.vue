...
<template>
    <section class="space-y-3">
        <div v-if="isLoading" class="rounded border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
            {{ label('pb_editor_iframe_loading', 'Loading editor...') }}
        </div>

        <div v-if="loadError" class="rounded border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
            {{ loadError }}
        </div>

        <div v-if="!iframeEnabled || iframeSrc === ''" class="rounded border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        ...
        <div class="rounded bg-white">
            <iframe
                ...
                ref="iframeRef"
                :src="iframeSrc"
                class="w-full rounded border border-slate-200 bg-white"
                :class="{
                    'min-h-[70vh]': !isFullscreenActive,
                    'min-h-screen': isFullscreenActive,
                }"
