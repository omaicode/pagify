...
<template>
    <AdminLayout>
        <div class="mb-4 flex items-center justify-between">
            <UiPageHeader :title="label('audit_logs', 'Audit logs')" />
        </div>

        <UiCard tag="form" class="mb-4 grid grid-cols-1 gap-2 p-3 md:grid-cols-5" @submit.prevent="submitFilters">
            <UiInput v-model="filterForm.q" type="text" :placeholder="label('search', 'Search')" />

            <UiInput v-model="filterForm.action" tag="select">
            ...
            <UiInput v-model="filterForm.entity_type" tag="select">
            ...
            <UiInput v-model="filterForm.entity_id" type="text" :placeholder="label('entity_id', 'Entity ID')" />

            <UiInput v-model="filterForm.site_id" tag="select">
            ...
            <UiInput v-model="filterForm.admin_id" tag="select">
            ...
            <UiInput v-model="filterForm.date_from" type="date" />
            <UiInput v-model="filterForm.date_to" type="date" />

            <UiInput v-model="filterForm.per_page" tag="select">
            ...
            <UiInput v-model="filterForm.sort_by" tag="select">
            ...
            <UiInput v-model="filterForm.sort_dir" tag="select">
                <option value="desc">{{ label('newest_first', 'Newest first') }}</option>
                <option value="asc">{{ label('oldest_first', 'Oldest first') }}</option>
            </UiInput>

            <div class="flex gap-2 md:col-span-5">
                <UiButton type="submit" radius="lg">{{ label('filter', 'Filter') }}</UiButton>
                <UiButton type="button" tone="neutral" radius="lg" @click="resetFilters">{{ label('reset', 'Reset') }}</UiButton>
            </div>
        </UiCard>
